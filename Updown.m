...
configurePin(Mega,'D3','DigitalOutput');      % Motor 5 Direction
configurePin(Mega,'D4','DigitalOutput');
configurePin(Mega,'D9','DigitalOutput');      % M5 enabled
configurePin(Mega,'D12','DigitalOutput');     % M3 enabled.
configurePin(Mega,'D6','DigitalOutput');      % M3 Direction
...
vid.FrameGrabInterval=2;
 ...
 while(vid.FramesAcquired<=Inf)                 %increase frames to Inf if you want the loop to be never ending
     ...
     imshow(diff_im)%if you want the color to be anything other than red then change the loop
